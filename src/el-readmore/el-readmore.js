...
customElements.define('el-readmore', class extends HTMLDivElement {
  ...
  addBtn() {
    ...
    const btn_classes = this.getAttribute('data-btn-class');
  ...
  update() {
    if (this.$btn) this.$btn.innerHTML = this.getBtnLabel(this.hasAttribute('data-expanded'));
    if (this.hasAttribute('data-expanded')) this.execInlineEvent('data-on:expand');
    else this.execInlineEvent('data-on:collapse');
  }
  ...
  getBtnLabel(expanded) {
    ...
    if (!labels || labels.length < 2) labels = _default;
