...
customElements.define('fm-imgprev', class extends HTMLInputElement {
  ...
  attributeChangedCallback(name, oldVal, newVal) {
    ...
    if (_name === 'data-width' || _name === 'data-height') {
      this.resize();
    }
    if (_name === 'data-placeholder') {
      this.updateLabel(this.dataset.placeholder);
    }
    if (_name === 'data-fake-btn') {
      this.updateFakeBtnText(this.dataset.fakeBtn);
    }
    if (_name === 'data-fake-btn-classes') {
  ...
  createFakeBtn() {
    ...
    const classes = this.dataset.fakeBtnClasses ? this.dataset.fakeBtnClasses.split(' ') : '';
      ...
      if (files && files[0]) {
        fr.onload = (_e) => {
          if (this.validateImgSize(files[0].size, minSize, maxSize)) {
            return this.updatePreviewImage(_e.target.result);
          }
          return false;
        };
        fr.onerror = (err) => {
  ...
  execInlineEvent(event) {
    ...
    return window[this.getAttribute(event).split('(')[0].trim()](this);
  }
}, { extends: 'input' });
